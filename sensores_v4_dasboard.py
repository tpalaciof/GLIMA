...
# El RG-15 se consulta cada 60 segundos.
...
valoresLluviaTotalesArchivo = 0
valoresLluviaCorrectosArchivo = 0
...
def enviarDatosThingsBoard(
    ...
    valores = {
        "lluvia_adicional_mm": lluviaAdicionalMm,
        "lluvia_evento_mm": lluviaEventoMm,
        "lluvia_total_mm": lluviaTotalMm,
        "intensidad_lluvia_mm_h": intensidadLluviaMmH,
        "distancia_moda_mm": distanciaModaMm,
        "porcentaje_datos_correctos_lluvia": porcentajeLluvia,
        "porcentaje_datos_correctos_distancia": porcentajeDistancia
    }

    # En el CSV un valor inexistente se guarda como una celda vacía.
    # En ThingsBoard simplemente se omite hasta que exista una medición.
    valores = {
        nombre: valor
...
def convertirNumeroValido(texto):
...
def esValorLluviaValido(valor):
    numero = convertirNumeroValido(valor)
    return numero is not None and numero >= 0


def esModaDistanciaValida(distanciaModaMm):
    numero = convertirNumeroValido(distanciaModaMm)

    return (
        numero is not None
        and 0 <= numero <= DISTANCIA_MAXIMA_MM
    )


def actualizarColumnasCsvAnterior(rutaCsv):
    ...
    if not filas or filas[0] == ENCABEZADOS_CSV:
        return False

    encabezadosOriginales = ENCABEZADOS_CSV[:7]
    encabezadosPorcentajeAnterior = [
        *encabezadosOriginales,
        "porcentaje_datos_correctos"
    ]
    ...
    filasActualizadas = [ENCABEZADOS_CSV]

    for fila in filas[1:]:
        filaBase = fila[:7]
        filaBase.extend([""] * (7 - len(filaBase)))
        filasActualizadas.append([*filaBase, "", ""])

    rutaTemporal = rutaCsv.with_suffix(rutaCsv.suffix + ".tmp")

    with rutaTemporal.open("w", newline="", encoding="utf-8") as archivo:
        csv.writer(archivo).writerows(filasActualizadas)

    rutaTemporal.replace(rutaCsv)
    return True


def cargarContadoresCalidadDesdeCsv(rutaCsv):
    global valoresLluviaTotalesArchivo
    global valoresLluviaCorrectosArchivo
    ...
    with rutaCsv.open("r", newline="", encoding="utf-8") as archivo:
        ...
        for fila in lector:
            for nombre in nombresLluvia:
                valoresLluviaTotalesArchivo += 1

                if esValorLluviaValido(fila.get(nombre)):
                    valoresLluviaCorrectosArchivo += 1

            valoresDistanciaTotalesArchivo += 1

            if esModaDistanciaValida(fila.get("distancia_moda_mm")):
...
def registrarCalidadFila(valoresLluvia, distanciaModaMm):
    global valoresLluviaTotalesArchivo
    global valoresLluviaCorrectosArchivo
    global valoresDistanciaTotalesArchivo
    global valoresDistanciaCorrectosArchivo

    valoresLluviaTotalesArchivo += len(valoresLluvia)
    valoresLluviaCorrectosArchivo += sum(
        esValorLluviaValido(valor)
        for valor in valoresLluvia
    )
    ...
    porcentajeLluvia = round(
        valoresLluviaCorrectosArchivo
        / valoresLluviaTotalesArchivo
        * 100,
        2
    )
    porcentajeDistancia = round(
    ...
    return porcentajeLluvia, porcentajeDistancia
...
def guardarDatos():
    ...
    columnasCsvActualizadas = actualizarColumnasCsvAnterior(rutaCsv)
    ...
    porcentajeLluvia, porcentajeDistancia = registrarCalidadFila(
        valoresLluvia,
        distanciaModaMm
    )
    ...
    if archivoTxtNuevo or columnasCsvActualizadas:
    ...
    # Si Internet falla, esta función no interrumpe los archivos locales.
    ...
    print(
        ...
        f"calidad de lluvia: {porcentajeLluvia:.2f}% | "
